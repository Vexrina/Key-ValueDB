...
def test_create_database(base_url, endpoint, database):
    response = httpx.post(
        base_url + endpoint["db"],
        json={"db_name": database},
    )
    assert response.status_code == 201, f"Unexpected status code: {response.status_code}"

def test_create_table(base_url, endpoint, database, table):
    response = httpx.post(
        base_url + endpoint["table"] + "?db_name="+database,
...
def test_put_table(base_url, endpoint, database, table):
    response = httpx.put(
        ...
        json={
            "table_name": table,
            "new_table_name": "table_database_new2",
        },
    )
    ...
    response = httpx.put(
        base_url + endpoint["table"] + "?db_name=" + database,
        json={
            "table_name": "table_database_new2",
...
def test_create_key(base_url, endpoint, database, table):
    ...
    response = httpx.post(
        base_url + endpoint["key"] + "?db_name=" + database+"&table_name="+table,
        json=json,
    )
    assert response.status_code == 200, f"Unexpected status code: {response.status_code}"
